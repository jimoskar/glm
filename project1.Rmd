...
Let $\mu = \mu(\beta_0,\beta_1)$ and $\sigma = \sigma(\beta_0,\beta_1)$. If $\hat\beta_0$ and $\hat\beta_1$ denotes the MLEs of $\beta_0$ and $\beta_1$, respectively, then $\hat\mu = \mu(\hat\beta_0,\hat\beta_1)$ is the MLE of $\mu$  and $\hat\sigma = \sigma(\hat\beta_0,\hat\beta_1)$ of $\sigma$. It follows that
$$
  \sigma = \frac{1}{\beta_1}, \quad \mu = -\frac{\beta_0}{\beta_1},
$$
Consequently, the MLEs are given as
$$
  \hat\sigma(\hat\beta_1) = \frac{1}{\hat\beta_1}, \quad \hat\mu(\hat\beta_0, \hat\beta_1) = -\frac{\hat\beta_0}{\hat\beta_1}.
$$
The corresponding standard errors of these estimators can then be estimated using the delta method. A first order Taylor expansion of $\hat{\mu}$ gives \textcolor{red}{Legge til mitt forslag under:}

$$
\hat\mu \approx \hat\mu(\boldsymbol{b}) + \nabla \hat\mu(\boldsymbol{b})^T(\hat{\boldsymbol{\beta}} - \boldsymbol{b}),
$$
where we have used the notation $\hat{\boldsymbol{\beta}} = (\hat{\beta}_0, \hat{\beta_1})^T$ and exoanded around $\hat{\boldsymbol{\beta}} = \boldsymbol{b} = (b_0,b_1)^T$. Next, we take the variance of this linear approximation, such that
...
se.mu
```
\textcolor{red}{et annerledes svar her altså. Hva tenker du ?}

$$
  \mu(\beta_0,\beta_1) \approx \hat\mu + \frac{\partial \mu}{\partial \beta_0}(\hat{\beta}_0, \hat{\beta}_1) \left(\beta_0 - \hat\beta_0\right) + \frac{\partial \mu}{\partial \beta_1} \left(\beta_1 - \hat\beta_1\right),
$$
so
$$
  \begin{split}
    \text{Var}(\mu) &\approx \left(\frac{\partial \mu}{\partial \beta_0}\right)^2 \text{Var}\left(\beta_0\right) + \left(\frac{\partial \mu}{\partial \beta_1}\right)^2 \text{Var}\left(\beta_1\right) + 2 \left(\frac{\partial \mu}{\partial \beta_0}\right) \left(\frac{\partial \mu}{\partial \beta_1}\right) \text{Cov}\left(\beta_0, \beta_1\right)\\
    & = \left(-\frac{1}{\beta_1}\right)^2 \text{Var}\left(\beta_0\right) + \left(\frac{\beta_0}{\beta_1^2}\right)^2 \text{Var}\left(\beta_1\right) - \frac{2 \beta_0}{\beta_1^3} \text{Cov}\left(\beta_0, \beta_1\right).\\
  \end{split}
$$

A similar derivation for $\sigma$ yields
$$
  \begin{split}
    \text{Var}(\sigma) &\approx \left(\frac{\partial \sigma}{\partial \beta_0}\right)^2 \text{Var}\left(\beta_0\right) + \left(\frac{\partial \sigma}{\partial \beta_1}\right)^2 \text{Var}\left(\beta_1\right) + 2 \left(\frac{\partial \sigma}{\partial \beta_0}\right) \left(\frac{\partial \sigma}{\partial \beta_1}\right) \text{Cov}\left(\beta_0, \beta_1\right)\\
    & = \left(-\frac{1}{\beta_1^2}\right)^2 \text{Var}\left(\beta_1\right).\\
  \end{split}
$$

Thus, the standard error of the MLE estimators are
$$
 \widehat{\text{SE}\left(\hat\sigma\right)} = \frac{1}{\hat\beta_1^2}\ \text{SE}(\hat\beta_1), \quad \widehat{\text{SE}(\hat\mu)} = \frac{1}{\hat\beta_1}\sqrt{\text{Var}(\hat\beta_0) + \left(\frac{\hat\beta_0}{\hat\beta_1}\right) \text{Var}(\hat\beta_1)},
$$
where it was used (assumed?) that $\text{Cov}(\beta_0, \beta_1) = 0$. In R we find the $\beta$'s and their standard error using the summary-function.

```{r}
mod.probit <- glm(menarche ~ age, family = binomial(link = 'probit'), data = juul.girl)
s <- summary(mod.probit)
df <- s$coefficients
df
beta0 <- df[1,1]
beta1 <- df[1,2]
...
From the readout we get $\hat\beta_0 =$ `r beta0` with a standard error of `r se0` and $\hat\beta_1 =$ `r beta1` with a standard error of `r se1`. Thus $\hat\mu =$ `r -beta0/beta1` and $\hat \sigma =$ `r 1/beta1`. Their standard errors are $\widehat{\text{SE}\left(\hat\sigma\right)} =$ `r 1/beta1^2 * se1` and $\widehat{\text{SE}\left(\hat\mu\right)} =$ `r 1/beta1 * sqrt(se0^2 + (beta0/beta1)*se1^2)`.
...
## e)
For now we assume that the link function is the cloglog, given by

$$
g(\pi) = \text{cloglog}(\pi) = \ln(-\ln(1-\pi)).
$$
In addition, our model only use log(age) as a covariate. Using the same notation as before, it follows that 

$$
  \eta_i = \beta_0 + \beta_1 \ln(t_i),
$$
where $t_i$ is the age. Thus

$$
  \mathrm{Pr}(T_i \le t_i) = \pi_i = g^{-1}(\eta_i) = 1 - e^{-e^{\eta_i}} = 1 - e^{-e^{\beta_0 + \beta_1}\ t_i} = 1 - e^{-\gamma t_i},
$$
where $\gamma = e^{\beta_0 + \beta_1}$. This is the cumulative distribution function of $T_i$, so the distribution of $T$ is given by

$$
  T \sim \frac{\partial}{\partial t} \pi = \frac{\partial}{\partial t} \left(1 - e^{-\gamma\ t}\right) = \gamma\ e^{-\gamma\ t},
$$
which is a Weibull distribution with shape parameter $k = 1$ and scale parameter $\lambda = \frac{1}{\gamma} = e^{-(\beta_0 + \beta_1)}$. That is,

$$
  T \sim \mathrm{Weibull}(t;\ e^{-(\beta_0+\beta_1)},\ 1) = e^{\beta_0 + \beta_1} e^{-e^{\beta_0 + \beta_1}\ t}.
$$
The estimate for the shape parameter is $\hat k = 1$. Using that $\boldsymbol{\beta} = (\beta_0,\ \beta_1)$. Then for a realization $\boldsymbol{\hat\beta} = (b_0,\ b_1)$, the estimate for the scale parameter is
