...
$$
where $\hat\beta_0$ and $\hat\beta_1$ denote the MLEs of $\beta_0$ and $\beta_1$, respectively. Thus, the maximum likelihood estimates for this data set can be computed as
...
The standard errors of $\hat{\mu}$ and $\hat{\sigma}$ can then be estimated using the delta method. A first order Taylor expansion of $\hat{\mu}$ gives
...
se.mu
```
That is, $\widehat{\mathrm{SE}(\hat{\mu})} =$ `r se.mu`. We follow the same procedure to estimate the standard error of $\hat{\sigma}$. First, we approximate it by a first order Taylor series expansion:
$$
...
se.sigma
```
That is, $\widehat{\mathrm{SE}(\hat{\sigma})} =$ `r se.sigma`.

\textcolor{red}{Q: are we estimating the standerd error (SE) or is the SE an estimate in itself? i.e. we dont need the hat?}
