...
On matrix notation, the score function can be written as
...
To test that our implementation of myglm works, we will simulate 1000 Bernoulli draws, each with a random probability $x_i \sim \text{Uniform}(0,\ 1)$. Then the response can be written as $y_i \sim \text{Bernoulli}(x_i)$. First we create the data set in R:
...
Next, we fit a model using myglm and print the betas, standard error, variance covariance matix and the deviance:
