...
$$
We now observe that $A$ is equivalent to $\{T_2 > t_{n'-2, \alpha}\} = \{T_2 > t_{n-p, \alpha}\}$, where $p = 3$ as before. To show that $A$ and $B$ are equivalent, we need to show that $T_1 = T_2$ Using profile log-likelihood, it can be shown that
...
$$
(X^TX)^{-1}_{2,2} = 1 + 1/n' + \frac{(x_h - \bar{x})^2}{\sum_{i = 1}^{n'}(x_i-\bar{x})^2},
$$
which shows that $T_1 = T_2$ and ehnce $A$ and $B$ are equivalent.

\textcolor{red}{Remains to show the above! I have not succeeded yet:()}
...
```
We see that the Wald test and the LRT test differ the most for the linear model. This could be explained by the fact that the Wald test for the lienar model is exact (the test statistic follows an $F$-distribution), while in the LRT test we use an asymptotic distribution. \textcolor{red}{unsure here.}

# f)
We calculate the AIC:
```{r}
p = 3
AIC.linear <- 2*p + 2*logLik(mod1)
AIC.gamma <- 2*p + 2*logLik(mod.gamma)
AIC.linear
AIC.gamma
```
We need to be careful comparing these, because the models have different distributional assumptions. \textcolor{red}{something else maybe?}
...
$$
\textcolor{red}{Need to drop one column from the design matrix in order to get full rank. Why?}
```{r}
library(Matrix)
