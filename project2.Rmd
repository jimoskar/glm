...
$$
P(T \le U) = 1 - \alpha \implies U = t_{n-2, \alpha}.
$$
Since there are $n =$ `r n-1` rows in the dataset (excluding humans), the exact prediction interval is $(-\infty, t_{\text{`r n-1`}, \alpha})$
...
$$
  ...
  \end{cases}
$$
which may depend on relative strength of different players, which player plays white and black and the type of game played. The response can be determined by an underlying latent variable $u_i$, where
$$
  u_i = x_i^T \boldsymbol{\beta} + \epsilon_i.
$$
The event $y_i = r$ occurs if $\theta_{r-1} < u_i \le \theta_{r}$, where
$$
