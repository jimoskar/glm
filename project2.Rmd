...
In our analysis, we will consider two different distributions of the data. Both 
...
### Liklihood ratio test based on change in deviance

For two nested models $M_i$ and $M_j$, where $M_i$ contains the parameters in $M_j$ and an additional $k$ parameters. Then under the null hypothesis that $M_i$ is the true model, then, based on Wilk's theorem, it follows that the change in deviance between the two models is approximately chi-squared distributed with $k$ degrees of freedom. This can be written as
$$
  \Delta D = D_i - D_j \overset{\text{approx}}{\sim} \chi^2_{k},
$$
which can later be used for hypothesis testing using the inbuilt pchisq()-function.
