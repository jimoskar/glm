...
\end{split}
$$
Since $x_{i,2}$ is nonzero for only one term in the sum above, say for  $i = h$, we only need to consider this term. That is, the term with $\boldsymbol{x}_h := \begin{bmatrix}1 & x_h & 1\end{bmatrix}^T$. The constant in front of $(y_h-\boldsymbol{x}_h^T\boldsymbol{\beta})^2$ is negative, so the supremum is when this is equal to zero. Thus,
