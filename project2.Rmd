...
```
The higher AIC and LR test suggests that this model does not perform any better.
...
```{r}
fit.interaction.round_nepo <- vglm(y ~ firouzja + rapport + nepo + karjakin + tari + abs(nepo):round,
                        family=cumulative(parallel = TRUE, link="logitlink"), data=df)
...
```
From the LR-tets, we see that there are evidence for rejecting `fit.simple` in favor of the new model including this interaction.

# better one ?
```{r}
fit.best <- vglm(y ~ I(firouzja + rapport) + nepo + karjakin + tari + karjakin:type,
                    family=cumulative(parallel = TRUE, link="logitlink"), data=df)
summary(fit.best)
c(AIC = AIC(fit.best), AICc = AICc(fit.best))
anova(fit.interaction.type_karjakin, fit.simple, type = 1, test = 'LRT')
```
